    ...
    then
      ...
      if [ ${atlas_enableTLS} == "true" ]; then
        if [ -z ${atlas_kafka_ssl_truststore_password} ] || [ -z ${truststore_password} ]; then
          ...
          # New configuration will be created at temporay directory
          NEW_ATLAS_APPLICATION_PROPERTIES="${scratch_dir}/atlas-application.properties"

          # Create the application properties file at s3 conf directory
...
function set_log_dir_file() {
  ...
  export LOGFILE="$ATLAS_LOG_DIR/aws-s3-extractor.log"
}

function construct_classpath() {
  # Construct Atlas classpath using jars from lib/aws-s3/ directory.
  for i in "${BASEDIR}/lib/aws-s3/"*.jar; do
...
#usage: S3ExtractorMain
# -c,--config <arg>       Configuration file for s3 extraction
# -e,--extraction <arg>   Extraction type to be done. Possible values are
#                         “INC” and “BULK” for incremental and Bulk
#                         extractions.
# -l,--logdir <arg>       Log directory for AWS S3 Extractor
# -f,failOnError          Fail when error is encountered
  case "$option" in
    ...
    -l) export ATLAS_LOG_DIR="$1"; export LOGFILE="$ATLAS_LOG_DIR/aws-s3-extractor.log"; shift;;
    ...
    --logdir) export ATLAS_LOG_DIR="$1"; export LOGFILE="$ATLAS_LOG_DIR/aws-s3-extractor.log"; shift;;
    "") break;;
    *) ARGS="$ARGS $option"
  esac
done
...
if [ -z ${HELP_OPTION} ]; then
  echo "Log file for Aws S3 Extraction is $LOGFILE"
fi

"${JAVA_BIN}" ${JAVA_PROPERTIES} -cp "${CP}" org.apache.atlas.s3.cli.S3ExtractorMain $ARGS

RETVAL=$?
if [ -z ${HELP_OPTION} ]; then
  [ $RETVAL -eq 0 ] && echo Aws S3 Meta Data extracted successfully!!!
  [ $RETVAL -eq 1 ] && echo Failed to extract S3 Meta Data!!!
